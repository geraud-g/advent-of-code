...
pub fn solve(
    plane: &Plane,
    get_value: fn(&Plane, char, usize, usize, usize, usize) -> char,
) -> usize {
    ...
    loop {
        for y in 0..height {
            for x in 0..width {
                new_plane.seats[y][x] =
                    get_value(&tmp_plane, tmp_plane.seats[y][x], y, x, width, height)
            }
        }
...
fn get_value_part_a(
    ...
    for (delta_y, delta_x) in DIRECTIONS.iter() {
        ...
        if is_valid_index(tmp_y, tmp_x, width, height)
            && tmp_plane.seats[tmp_y as usize][tmp_x as usize].eq(&OCCUPIED)
        {
            occupied_seats += 1
        }
    }
