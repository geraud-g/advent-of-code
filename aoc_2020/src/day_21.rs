...
fn get_input() -> Vec<(HashSet<Ingredient>, HashSet<Allergen>)> {
    ...
    for line in get_file("./inputs/day_21.txt").lines() {
        ...
        let foods: HashSet<String> = matches
            .get(1)
            .unwrap()
            .as_str()
            .split(' ')
            .map(|l| l.to_string())
            .collect();
        let allergens: HashSet<String> = matches
            .get(2)
            .unwrap()
            .as_str()
            .split(", ")
...
fn solve_part_a(foods: &[(HashSet<Ingredient>, HashSet<Allergen>)]) -> (usize, String) {
    ...
    let mut safe_ingredients = foods
        .iter()
        .flat_map(|(ingredient, _)| ingredient)
        .cloned()
        .collect();
    let allergens: &HashSet<Allergen> = &foods
        .iter()
        .flat_map(|(_, allergen)| allergen)
        .cloned()
        .collect();
    ...
    (
        get_solution_part_a(&safe_ingredients, foods),
        get_solution_part_b(allergen_map, allergens),
    )
}

fn get_allergen_ingredient(
    allergen: &str,
    food_list: &HashSet<Ingredient>,
    foods: &[(HashSet<Ingredient>, HashSet<Allergen>)],
) -> Option<Ingredient> {
    ...
    {
        remaining_suspect_food = remaining_suspect_food
...
fn get_solution_part_a(
    safe_ingredients: &HashSet<Ingredient>,
    foods: &[(HashSet<Ingredient>, HashSet<Allergen>)],
) -> usize {
    let mut counter = 0;
    for safe_ingredient in safe_ingredients {
        counter += foods
            .iter()
            .filter(|(ingredients, _)| ingredients.contains(safe_ingredient))
            .count();
    }
    counter
}

fn get_solution_part_b(
    allergen_map: HashMap<&Allergen, Ingredient>,
